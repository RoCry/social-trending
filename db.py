...
class Database:
    ...
    async def upsert_item(self, item: Dict[str, Any]) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                f"SELECT created_at FROM {ITEM_TABLE_NAME} WHERE id = ?",
                (item["id"],)
            ) as cursor:
    ...
    async def get_item(self, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                f"SELECT payload FROM {ITEM_TABLE_NAME} WHERE id = ?",
                (item_id,)
            ) as cursor:
    ...
    async def cleanup(self, before_days: int = 30) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                ...
                await db.commit()
                logger.info(f"Cleaned up {deleted_count} items older than {before_days} days")
